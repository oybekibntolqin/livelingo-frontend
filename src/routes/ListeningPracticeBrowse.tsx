...
import UploadAudioModal from '../components/listening/UploadAudioModal'
import UploadWithQuestionsModal from '../components/listening/UploadWithQuestionsModal'
...
export default function ListeningPracticeBrowse() {
  ...
  // Foydalanuvchining o'z LEARNING tillari — upload modallarida til
  // tanlashni shu ro'yxatga cheklash uchun (bug tuzatish: avval
  // butun til ro'yxatidan istalganini tanlash mumkin edi).
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((langs) => {
        ...
        setLearningLangs(learning.map((l) => l.languageCode))
  ...
  // Upload modallarida ko'rsatiladigan, faqat foydalanuvchining o'z
  // learning tillariga cheklangan ro'yxat (hali yuklanmagan bo'lsa —
  // to'liq ro'yxatga tushib qolamiz).
  const uploadLangOptions = useMemo(
      () =>
  ...
  return (
    <main className="min-h-screen bg-cream">
        ...
        <section>
          ...
          <div className="mb-5">
            ...
            <p className="mt-1 text-sm text-ink-soft">
              Upload your material and work on the AI assessment
            </p>
          </div>

          {/* Search + Upload tugmalar */}
          <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            {/* Upload tugmalar — 2 ta alohida flow */}
      ...
      {/* Modal 1 — AI generation flow */}
      <UploadAudioModal
        open={aiModalOpen}
        onClose={() => setAiModalOpen(false)}
        onCreated={(m) => {
          setAiModalOpen(false)
          navigate(`/learn/listening/practice/${m.id}`)
        }}
        defaultLanguage={lang}
        allowedLangCodes={learningLangs}
      />

      {/* Modal 2 — User o'z savollari bilan flow */}
      <UploadWithQuestionsModal
        open={ownModalOpen}
...
function MaterialCard({ material: m }: { material: ListeningMaterial }) {
  return (
    ...
    >
      <Link
        to={`/learn/listening/practice/${m.id}`}
